...
@test "Engram succeeds standalone and warns when Pi is absent" {
...
@test "Engram configures Pi and preserves integration failures when Pi exists" {
	mkdir -p "${TEST_ROOT}/home" "${TEST_ROOT}/path"
...
@test "ENGRAM_SETUP_PI=0 skips integration even when Pi exists" {
	mkdir -p "${TEST_ROOT}/home" "${TEST_ROOT}/path"
	printf '#!/usr/bin/env bash\nexit 0\n' >"${TEST_ROOT}/path/pi"
	chmod +x "${TEST_ROOT}/path/pi"
	run env HOME="${TEST_ROOT}/home" PATH="${TEST_ROOT}/path:/usr/bin:/bin" DEVCONTAINER_PHASE=runtime \
		ENGRAM_SETUP_PI=0 ENGRAM_INSTALL_DIR="${INSTALL_DIR}" ENGRAM_PROFILE_FILE="${TEST_ROOT}/profile" \
		ENGRAM_DATA_DIR="${TEST_ROOT}/data" ENGRAM_CALLS_FILE="${CALLS_FILE}" \
		DEVCONTAINER_TOOL_VERSIONS_FILE="${TEST_ROOT}/tool-versions.conf" \
		bash "${TEST_ROOT}/install/available/30-ai-engram.sh"
	[ "$status" -eq 0 ]
