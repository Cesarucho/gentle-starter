#!/usr/bin/env bash
#
# 2060-node-test.sh — vitest for Node.js testing.
#
# REQUIRES: 2000-runtime-node.sh (node must be installed first)
#
...
devcontainer_require_cmd npm "Enable 2000-runtime-node.sh before Vitest." || exit 1

# Guard: skip if node is not present (this tool depends on node being installed).
if ! devcontainer_has_cmd node; then
	devcontainer_log_warn "Skipping vitest: node is not installed. Verify mandatory 2000-runtime-node.sh activation."
	exit 0
fi
