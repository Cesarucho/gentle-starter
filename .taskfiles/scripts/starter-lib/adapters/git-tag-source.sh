#!/usr/bin/env bash
# Git-only release source adapter. Fetched content is treated strictly as data.
...
git_tag_enforce_evidence_limits() {
	local repository="$1" limits="$2" closure_file="$3"
	...
	jq -e 'type == "object" and (keys | sort) == ["max_object_bytes","max_pack_bytes","max_reachable_objects","max_retained_bytes"] and
		all(.[]; type == "number" and floor == . and . > 0)' <<<"${limits}" >/dev/null || {
		git_tag_source_error "evidence limits are invalid"
		return 1
	}
	object_limit="$(jq -r '.max_reachable_objects' <<<"${limits}")"
	object_bytes_limit="$(jq -r '.max_object_bytes' <<<"${limits}")"
	pack_limit="$(jq -r '.max_pack_bytes' <<<"${limits}")"
	retained_limit="$(jq -r '.max_retained_bytes' <<<"${limits}")"
	[ "${object_count}" -le "${object_limit}" ] || {
		git_tag_source_error "reachable object count exceeds evidence limit"
		return 1
	}
	[ "${max_object_bytes}" -le "${object_bytes_limit}" ] || {
		git_tag_source_error "per-object size exceeds evidence limit"
		return 1
	}
	[ "${pack_bytes}" -le "${pack_limit}" ] || {
		git_tag_source_error "pack size exceeds evidence limit"
		return 1
	}
	[ "${retained_bytes}" -le "${retained_limit}" ] || {
		git_tag_source_error "aggregate retained bytes exceed evidence limit"
		return 1
	}
}

git_tag_source_acquire() (
	local request_file="$1" remote selector output_dir source_id output_parent temporary limits
	local repository manifest evidence index_sha source_identity release_identity content_identity envelope_sha
	trap '[ -z "${temporary:-}" ] || rm -rf "${temporary}"' EXIT
	jq -e 'type == "object" and
		(keys | sort) == ["output_dir","remote","schema","selector","source_id"] and
		.schema == "gentle-starter.git-tag-source-request/v1" and
		all(.remote,.selector,.output_dir,.source_id; type == "string" and length > 0)' "${request_file}" >/dev/null || {
	...
	limits="$(starter_evidence_limits)" || return 1
	...
	release_payload_validate "${temporary}/envelope.json" "${temporary}/materialized" || return 1
...
git_tag_evidence_revalidate() (
	local evidence="$1" root repository index envelope materialized expected actual temporary count index_position oid type bytes limits
	...
	limits="$(jq -c '.evidence_limits' "${index}")" || return 1
	git_tag_enforce_evidence_limits "${repository}" "${limits}" "${index}" || return 1
	git --git-dir="${repository}" fsck --full --strict --no-dangling >/dev/null 2>&1 || {
